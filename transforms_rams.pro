...
function galradec, long, lat, inverse=inverse, silent=silent
  sil = keyword_set(silent)
  error = 'Something is wrong with your input to GALRADEC'
  ...
  if inv then begin
     ; if inv then "long" = ra and "lat" = dec
     precess, long, lat, 2016, 2000
  endif  
  lat = float(lat)*!dtor
  ...
  if not inv then begin
     precess, ra, dec, 2000, 2016
  endif
  if not sil then begin
...
function radechadec, ra, dec, inverse=inverse, hours=hours, silent=silent
  sil = keyword_set(silent)
...
function hadecazalt, ha, dec, nlat=nlat, hours=hours, inverse=inverse, silent=silent
  sil = keyword_set(silent)
...
function galhadec, long, lat, silent=silent
  return, radechadec(galradec(long, lat, silent=silent), silent=silent)
end

function galazalt, long, lat, silent=silent
  return, hadecazalt(galhadec(long, lat, silent=silent), silent=silent)
end

function radecazalt, ra, dec, silent=silent
  return, hadecazalt(radechadec(ra, dec, silent=silent), silent=silent)
end

function azalthadec, az, alt, silent=silent
  return, hadecazalt(az, alt, /inverse, silent=silent)
end

function azaltradec, az, alt, silent=silent
  return, radechadec(azalthadec(az, alt), /inverse, silent=silent)
end

function azaltgal, az, alt, silent=silent
  return, galradec(azaltradec(az, alt, silent=silent), /inverse, silent=silent)
end

function hadecradec, ha, dec, silent=silent
  return, radechadec(ha, dec, /inverse, silent=silent)
end

function hadecgal, ha, dec, silent=silent
  return, galradec(hadecradec(ha, dec, silent=silent), /inverse, silent=silent)
end

function radecgal, ra, dec, silent=silent
  return, galradec(ra, dec, /inverse, silent=silent)
end
